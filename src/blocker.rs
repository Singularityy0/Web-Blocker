...
impl BlockerState {
    ...
    // Build the full set of all domain variants currently tracked.
    // Called BEFORE mutating the site lists so the correct set is passed to
    // hosts::clean().
    ...
    pub fn update_hosts(&mut self) -> io::Result<()> {
        ...
        // Pass all currently tracked domains so clean() knows what to remove.
    ...
    /// Remove a domain and update the hosts file.
    /// The hosts file is cleaned BEFORE the domain is removed from the in-memory
    /// set, so that hosts::clean() still knows which entries to strip out.
    pub fn remove_and_update(&mut self, domain: &str) -> Result<bool, String> {
        // Check it actually exists first.
        let in_permanent = self.sites.permanent_sites.contains(domain);
        let in_timed     = self.sites.timed_sites.contains_key(domain);
        if !in_permanent && !in_timed {
            return Ok(false);
        }

        // 1. Clean + rewrite hosts BEFORE touching the in-memory set, so
        //    all_tracked_domains() still contains this domain and clean() can
        //    find and remove its entries from the file.
        self.update_hosts()
            .map_err(|e| format!("Failed to update hosts file: {}", e))?;

        // 2. Now remove from memory and persist JSON.
        self.sites.permanent_sites.remove(domain);
        self.sites.timed_sites.remove(domain);
        self.save()
            .map_err(|e| format!("Failed to save site list: {}", e))?;
